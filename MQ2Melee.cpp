				...
				if (id)
					for (unsigned long nAbility = 0; nAbility < AA_CHAR_MAX_REAL; nAbility++)
						if (long AAIndex = AAList[nAbility].AAIndex)
							if (PALTABILITY ability = GetAAByIdWrapper(AAIndex))
...
int AAReady(unsigned long index) {
	...
	{
		if (index)
			if (PALTABILITY ability = GetAAByIdWrapper(index))
			{
				//unsigned long i = 0;
...
int Equip(unsigned long ID, long SlotID)
{
	...
	if (GetItemFromContents(fITEM)->RequiredLevel > (int)GetCharInfo2()->Level)                            return false;
...
void Configure() {
	...
	// 2 hand bash fix 9/25/16 mackal
...
void MeleeHandle()
{
	...
	if (BerserkerClass && PressDelay < GetTickCount642())
	{
		Ability *UseThis = NULL;
		PressDelay = 0;
		...
		{
			UseThis->Press();
			PressDelay = GetTickCount642() + 1000;
		}
	}
	// If Monk do discs and attacks
	if (MonkClass && PressDelay < GetTickCount642())
	{
		Ability *UseThis = NULL;
		PressDelay = 0;
		...
		{
			UseThis->Press();
			PressDelay = GetTickCount642() + 500;
		}
	}
